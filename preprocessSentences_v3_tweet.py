...
def main(argv):
  ...
  testtxt = path+"/clean_tweet_reduced.txt"
  print ('Path:', path)
  print ('Testing data:', testtxt)
  ...
  if (not vocabf):
    ...
    vocabf = outputf+"_vocab_test_"+str(word_count_threshold)+".txt"
    outfile = codecs.open(path+"/"+vocabf, 'w',"utf-8-sig")
    outfile.write("\n".join(vocab))
    outfile.close()
  else:
    word_count_threshold = 0
    (docs, classes, samples) = tokenize_corpus(testtxt, train=False)
  ...
  # Write bow file
  with open(path+"/"+outputf+"_bag_of_words_tweet_"+str(word_count_threshold)+".csv", "w") as f:
    writer = csv.writer(f)
    writer.writerows(bow)

  # Write classes
  outfile = open(path+"/"+outputf+"_classes_tweet_"+str(word_count_threshold)+".txt", 'w')
  outfile.write("\n".join(classes))
  outfile.close()

  # Write samples
  outfile = open(path+"/"+outputf+"_samples_class_tweet_"+str(word_count_threshold)+".txt", 'w')
  outfile.write("\n".join(samples))
  outfile.close()
